...
(() => {
  ...
  if (!performance || typeof performance.getEntriesByType === 'function') return performance;
  if (typeof performance.now !== 'function') throw new Error('The native monotonic performance clock is unavailable');
  const now = performance.now.bind(performance);
  const clone = typeof target.structuredClone === 'function'
    ? target.structuredClone.bind(target) : globalThis.structuredClone.bind(globalThis);
  ...
  Object.defineProperty(globalThis, '__webSceneInstallPerformanceTimeline', {
    value: install, configurable: true
  });
  install(globalThis);
})();
)JS";
