...
int main()
{
    ...
    if (const auto* filter = std::getenv("WEBSCENE_NATIVE_ENGINE_TEST_FILTER");
        ...
        if (selected == "overscroll-performance") {
            auto* focused_engine = webscene_engine_create(0);
            require(focused_engine != nullptr,
                "overscroll performance engine creation failed");
            test_overscroll_behavior_performance_gate(focused_engine);
            webscene_engine_destroy(focused_engine);
            return 0;
        }
        if (selected == "css-isolation-performance") {
            auto* focused_engine = webscene_engine_create(0);
            require(focused_engine != nullptr,
                "CSS isolation performance engine creation failed");
            test_css_isolation_performance_gate(focused_engine);
            webscene_engine_destroy(focused_engine);
            return 0;
        }
        if (selected == "css-will-change-performance") {
            auto* focused_engine = webscene_engine_create(0);
            require(focused_engine != nullptr,
                "CSS will-change performance engine creation failed");
            test_css_will_change_performance_gate(focused_engine);
            webscene_engine_destroy(focused_engine);
            return 0;
        }
